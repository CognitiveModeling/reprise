...
def actinf():
    ...
    # CONSTANTS
    NUM_ALL_STEPS = 2000  # Amount of epochs of size NUM_MINI_EPOCHS
    NUM_TIME_STEPS = 5
    ...
    for t in range(NUM_ALL_STEPS):
        ...
        # sensor_data = a.data.sensors.get(from_step-2)
        # a.sensorplot.update(sensor_data, np.zeros_like(sensor_data), "t = " + str(from_step) + " + " +str(t+1))
        # plotname = a.id + "_" + str(from_step) + "+" + str(t)
        ...
        for t in range(len(a.actinf_sensor_predictions)):
            for a in agents:
                ...
                if a.show_sensor_plot and c.INPUT_SENSOR_DIM > 0:
                    ...
                    # Performance issues: only draw if sensor data changed
                    a.sensorplot.update(sensor_data, a.actinf_sensor_predictions[t], "t = " + str(from_step) + " + " +str(t+1))
                    plotname = a.id + "_" + str(from_step) + "+" + str(t)
                    a.sensorplot.save(plotname)
        ...
        # gui.save_screenshot(str(from_step))
    ...
    for a in agents:
        # a.plot.show()
        # a.plot.save('actinf')
        # np.savetxt("./results/actinf_agent" + str(a.id) + "_performance.csv", a.performances, delimiter=";")
        # np.savetxt("./results/actinf_stage" + get_stage() + "_agent" + str(a.id) + "_mode" + str(c.MODE) + ".csv", np.asarray(a1.performances2D), delimiter=";")

        filename = "actinf_distribution_stage" + get_stage() + "_agent" + str(a.id) + "_mode" + str(c.MODE)
