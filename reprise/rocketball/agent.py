...
class Agent(object):
    ...
    '''
    def actinf(self, from_step, to_step, learning_rate):
        ...
        predictions_abs_position = torch.tensor(abs_pos, dtype=torch.float) + torch.cumsum(torch.stack(self.actinf_position_predictions), dim=0)

        # Update GUI attributes
        if self.gui_att is not None:
            # GUI red line
            if self.gui_att.show_predictions:
            ...
            if self.gui_att.show_simulated_positions:
                ...
                motor_commands = torch.stack(self.actinf_inputs).data.numpy()[:, -1, -1, c.MOTOR_DIM_START:c.MOTOR_DIM_END]  # 2:6

                _, real_positions, _ = self.sim.simulate_multiple(previous_position, previous_velocity, motor_commands, self, from_step)
        ...
        if c.INPUT_SENSOR_DIM > 0:
            ...
            if c.USE_SCV is True and self.seek_proximity is False:
                ...
                for t in range(num_time_steps):
                    ...
                    scv = c.SCV_SMOOTHING_FACTOR * previous_scv + (1.-c.SCV_SMOOTHING_FACTOR) * c.SCV_WEIGHTING_FACTOR * np.sum(scvs[t], axis=0)
        ...
        if c.CLAMP_TARGET_VELOCITY:
            ...
            for t in range(len(vel_length)):
                if vel_length[t] < -clip_value:
                    targets_delta_position[t] = -clip_value * targets_delta_position[t] / np.linalg.norm(targets_delta_position[t])
                if vel_length[t] > clip_value:
                    targets_delta_position[t] = clip_value * targets_delta_position[t] / np.linalg.norm(targets_delta_position[t])
            ...
            if self.seek_proximity is True:
                ...
                for t in range(len(sensor_targets)):
                    ...
                    for sensor_index in range(c.INPUT_SENSOR_DIM):
                        point_spread_signal_tmp = point_spread(sensor_index, sensor_targets[t, sensor_index], c.POINT_SPREAD_TYPE)
        ...
        # Calculate gradients **for motor data**
        optimizer.zero_grad()
        loss.backward()

        # mask the gradients
        mask_gradients_(self.actinf_inputs)
        ...
        # Optimize motorcommands
        optimizer.step()
        grads = [self.actinf_inputs[i].grad for i in range(len(self.actinf_inputs))]
